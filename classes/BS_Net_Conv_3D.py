...
@ Description: 
-------------
Band selection networks based on convolution neural networks with 3D kernels.
-------------
...
class Attention_CNN:
    ...
    def net(self, x_input, is_training=True):
        ...
        # # attention net
        conv_att_1 = tf.layers.conv3d(input_norm, 32, (5, 3, 3), strides=(1, 1, 1), padding='valid',
        ...
        global_pool = tf.reduce_mean(bn_att_1, axis=[2, 3, 4], name='global_pooling')
        ...
        channel_weight_ = tf.reshape(channel_weight, [-1, n_channel, 1, 1, 1], name='weight_reshape')
        reweight_out = channel_weight_ * input_norm

        # # conv net
        conv_1 = tf.layers.conv3d(reweight_out, 32, (5, 3, 3), strides=(1, 1, 1), padding='valid',
        ...
        conv_2 = tf.layers.conv3d(batch_norm_1, 32, (5, 3, 3), strides=(1, 1, 1), padding='valid',
        ...
        conv_tr_1 = tf.layers.conv3d_transpose(batch_norm_2, 32, (5, 3, 3), strides=(1, 1, 1), padding='valid',
        ...
        conv_tr_2 = tf.layers.conv3d_transpose(conv_tr_bn_1, 32, (5, 3, 3), strides=(1, 1, 1), padding='valid',
        ...
        output = tf.layers.conv3d(conv_tr_bn_2, 1, (1, 1, 1), strides=(1, 1, 1), padding='same',
    ...
    def fit(self, X, img=None, gt=None):
        n_sam, n_depth, n_row, n_clm, n_channel = X.shape
        self.x_placehoder = tf.placeholder(shape=(None, n_depth, n_row, n_clm, 1), dtype=tf.float32)
        ...
        for i_epoch in range(self.epoch):
            ...
            weight_batch = np.zeros((n_sam, n_depth))
            loss_batch = 0
            batch_val = 500
...
if __name__ == '__main__':
    ...
    img_block = view_as_windows(X_img, (8, 8, n_band), step=1)
    ...
    img_block = np.reshape(img_block, (img_block.shape[0], img_block.shape[1], img_block.shape[2], img_block.shape[3], 1))
