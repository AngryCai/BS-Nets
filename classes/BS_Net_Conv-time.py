...
class Attention_CNN:
    ...
    def net(self, x_input, is_training=True):
        ...
        # # conv net
        conv_1 = tf.layers.conv2d(reweight_out, 64, (3, 3), strides=(1, 1), padding='valid',
        ...
        conv_2 = tf.layers.conv2d(batch_norm_1, 32, (3, 3), strides=(1, 1), padding='valid',
        ...
        conv_tr_1 = tf.layers.conv2d_transpose(batch_norm_2, 32, (3, 3), strides=(1, 1), padding='valid',
        ...
        conv_tr_2 = tf.layers.conv2d_transpose(conv_tr_bn_1, 64, (3, 3), strides=(1, 1), padding='valid',
    ...
    def fit(self, X, img=None, gt=None):
        ...
        for i_epoch in range(self.epoch):
            for batch_i in range(n_sam // self.batch_size):
                ...
                _, loss_ = sess.run([train_op, self.loss_recons],
                                    feed_dict={self.x_placehoder: x_batch, self.is_training: True})
                print('%s epoch, %s batch: %s' % (i_epoch, batch_i, loss_))
            run_time = round(time.clock() - time_start, 3)
            time_list.append(run_time)
        np.savez('run-time-epoch-conv.npz', run_time=np.asarray(time_list))
