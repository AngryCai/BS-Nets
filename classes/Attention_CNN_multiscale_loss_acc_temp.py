...
class Attention_CNN:
    ...
    def net(self, x_input, is_training=True):
        ...
        # # attention net
        global_pool = tf.reduce_mean(input_norm, axis=[1, 2], name='global_pooling')  # tf.keras.layers.GlobalAveragePooling2D(input_norm)
        ...
        channel_weight = tf.layers.dense(bottleneck, n_channel, activation=tf.nn.sigmoid,
                                         ...
                                         activity_regularizer=l2_regularizer(0.01), name='channel_weight')
    ...
    def fit(self, X, X_2, img=None, gt=None):
        ...
        for i_epoch in range(self.epoch):
            ...
            if img is not None:
                ...
                band_indx = np.argsort(mean_weight)[-self.n_selected_band:]
                print('=============================')
                ...
                score = eval_band_cv(img_correct, gt_correct, times=10, test_size=0.95)
                print('acc=', score)
...
if __name__ == '__main__':
    ...
    img_block = view_as_windows(X_img, (8, 8, n_band), step=3)
    ...
    acnn.fit(img_block, img_block_2, img=X_img, gt=gt)
