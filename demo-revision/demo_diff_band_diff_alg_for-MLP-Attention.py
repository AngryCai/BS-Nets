...
path = 'F:\Python\DeepLearning\AttentionBandSelection\\results\history-MLP-Indian-100-5band-0.01L2.npz'
npz = np.load(path)
# s = npz['score']
# s1 = npz['score'][:, 0, 0, 0]
# s2 = npz['score'][:, 1, 0, 0]
...
gt_1D = gt.reshape(-1)
acc_list = []
acc_dict = {'Attention_BS_FC':[]}
band_range = np.arange(3, 30, 2)
# alg_key = ['Attention_BS', 'ISSC', 'SpaBS', 'MVPCA', 'SNMF']
alg_key = ['Attention_BS_FC']
for i in band_range:
    alg = {
        'Attention_BS_FC': Attention_BS(w, i),
...
import matplotlib.pyplot as plt
import numpy as np
p_conv = 'F:\Python\DeepLearning\AttentionBandSelection\\results\diff_band_diff_alg-acc-IndianPines-[3-30]-added-FC-MOBS.npz'
...
score_conv['Attention_BS_FC'] = acc_dict['Attention_BS_FC']
np.savez('F:\Python\DeepLearning\AttentionBandSelection\\results\diff_band_diff_alg-acc-IndianPines-[3-30]-added-FC-MOBS-2.npz',
...
CLASSIFIER = 'svm'  # 'knn'
INDICATOR = 'oa'  # 'kappa'
p = 'F:\Python\DeepLearning\AttentionBandSelection\\results\diff_band_diff_alg-acc-PaviaU-[3-30]-added-FC.npz'
a = np.load(p)
score = a['res'][()]
alg_key = ['Attention_BS', 'Attention_BS_FC',  'ISSC', 'SpaBS', 'MVPCA', 'SNMF']
plot_key = ['BS-Net-Conv', 'BS-Net-FC', 'ISSC', 'SpaBS', 'MVPCA', 'SNMF']
marker = ['o', 'v', 's', 'd', 'H', '*', '^']
fig, ax = plt.subplots()
x_ = np.arange(3, 30, 2)
j = 0
